# function to perform many WA with increasing number of taxa ranked accoring to species importance
# in this function increases the size of the training set when more species are included
# Lukas Jonkers, last edit 19 Dec 2017

manyWA <- function(x){ # x is name of domain
  require(rioja)
  require(fields)
  ...
  rank <- row.names(rank.WA[[which(names(rank.WA) == x)]])
  ...
  for(i in 2:length(rank)){
    ...
    stat <- performance(cv)$crossval[1,] # h-block
    res[i-1,] <- stat
